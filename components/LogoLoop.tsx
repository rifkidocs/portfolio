  ...
  ({
    ...
    const renderLogoItem = useCallback(
      (item: LogoItem, key: React.Key) => {
        ...
        // Type guards for href property
        const hasHref = (
          item: LogoItem
        ): item is LogoItem & { href: string } => {
          return "href" in item && typeof item.href === "string";
        };

        const getAriaLabel = (item: LogoItem): string | undefined => {
          if (isNodeItem) {
            const nodeItem = item as LogoItem & {
              ariaLabel?: string;
              title?: string;
            };
            return nodeItem.ariaLabel ?? nodeItem.title;
          } else {
            const imgItem = item as LogoItem & { alt?: string; title?: string };
            return imgItem.alt ?? imgItem.title;
          }
        };

        const itemHref = hasHref(item) ? item.href : undefined;
        const itemAriaLabel = getAriaLabel(item);
        ...
        const content = isNodeItem ? (
          <span
            ...
            aria-hidden={!!itemHref && !itemAriaLabel}
          >
